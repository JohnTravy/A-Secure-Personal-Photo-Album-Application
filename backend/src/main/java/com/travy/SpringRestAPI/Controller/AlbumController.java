...
import com.travy.SpringRestAPI.Payload.Auth.Album.AlbumPayloadDTO;
import com.travy.SpringRestAPI.Payload.Auth.Album.AlbumViewDTO;
import com.travy.SpringRestAPI.Payload.Auth.Album.PhotoDTO;
import com.travy.SpringRestAPI.Payload.Auth.Album.PhotoPayloadDTO;
import com.travy.SpringRestAPI.Payload.Auth.Album.PhotoViewDTO;
import com.travy.SpringRestAPI.Service.AccountService;
import com.travy.SpringRestAPI.Service.AlbumService;
import com.travy.SpringRestAPI.Service.PhotoService;
import com.travy.SpringRestAPI.Util.Constraints.AlbumError;
import com.travy.SpringRestAPI.Utils.AppUtil;
...
public ResponseEntity<AlbumViewDTO>addAlbum(@Valid @RequestBody AlbumPayloadDTO albumPayloadDTO, Authentication authentication){
    ...
    @GetMapping(value = "/loalbums", produces = "application/json")
    @ApiResponses(value = {
    @ApiResponse(responseCode = "400", description = "please add valid name and Description"),
    @ApiResponse(responseCode = "200", description = "Account added")
})
            ...
            List<AlbumViewDTO>albums = new ArrayList<>();
             ...
             for(Album album : albumService.findByAccount_Id(account.getId())){
                ...
                for(Photo photo : photoService.findByAlbum_id(album.getId())){
                    ...
                    String link = "album/" + album.getId() + "/photos/" + photo.getId() + "/download_photo";
    ...
    public ResponseEntity<List <HashMap<String, List<?>>>>  photos (@RequestPart("files")MultipartFile [] files, @PathVariable Long album_id, Authentication authentication){
        ...
        Arrays.asList(files).stream().forEach(file->{
            ...
            || contentType != null && contentType.equals("image/jpg")
            || contentType != null && contentType.equals("image/jpeg")) {
                ...
                try {
                    ...
                    String generatedString = RandomStringUtils.random(count, 0, 0, useLetters, useNumbers, null, new Random());
    ...
    public ResponseEntity<?> downloadFile(Long album_id, Long photo_id, String thumbnailFolderName, Authentication authentication) {
